  ...
  for ( i in 1:length(cell.list)) ## any cell list
  {
    ...
    xhs<-try(lcs[id2,],TRUE)
     ...
     plot.hist.one.distribution.scale(x=xhs$mean.dir,
         xlab=expression(paste(Delta," HD pref. (deg)")),ylab="Shuffles",vline=xh$mean.dir)
     index=index+1
     
     screen(index)
     plot.hist.one.distribution.scale(x=xhs$vectorLength.diff,
                                      xlab=expression(paste(Delta," HD score")),ylab="Shuffles",vline=xh$vectorLength.diff)
     index=index+1
...
print(paste("Plot polar plots, spatial firing rate maps and changes in pref. HD, HD score and rate in  ",ep@resultsDirectory,"/plot_example_cells_hd.pdf",sep=""))
plot.page.full.maps(fn=paste(ep@resultsDirectory,"plot_example_cells_hd.pdf",sep="/"),
                    cell.list=cellid, 
                    cells1=cells1,
                    cells2=cells2,
                    tuning.hd1=tuning.hd1,
